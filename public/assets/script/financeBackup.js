...
function problem_tr(e){
  ...
  $('#problem-title', aside).html(data.problem_name)
  $('#status-problem', aside).html(data.problem_status)
  $('#problem-description', aside).html(`${getSafeArrayIndex(data.problem_description.match(regex),1)}`)
  $('#problem-cause', aside).html(`${getSafeArrayIndex(data.problem_cause.match(regex),1)}`)
  $('#problem-action', aside).html(`${getSafeArrayIndex(data.problem_action.match(regex),1)}`)
  $('#problem-who', aside).html(`${getSafeArrayIndex(data.problem_who.match(regex),1)}`)
  
  for (const key in data.problem_users){
     //responsible
    if(data.problem_users[key].user_type ==  1){
      $('#responsible', aside).prepend(`<li>${data.problem_users[key].user_name}</li>`)

    }else if(data.problem_users[key].user_type ==  2){
      $('#home-area', aside).prepend(`<li>${data.problem_users[key].user_name}</li>`)

    }else if(data.problem_users[key].user_type ==  3){
      $('#observer', aside).prepend(`<li>${data.problem_users[key].user_name}</li>`)
    } 
  }

  for (const key in data.problem_groups){
     //responsible
    if(data.problem_groups[key].group_type ==  1){
      $('#responsible', aside).prepend(`<li>${data.problem_groups[key].group_name}</li>`)

    }else if(data.problem_groups[key].group_type ==  2){
      $('#home-area', aside).prepend(`<li>${data.problem_groups[key].group_name}</li>`)

    }else if(data.problem_groups[key].group_type ==  3){
      $('#observer', aside).prepend(`<li>${data.problem_groups[key].group_name}</li>`)
    } 
  }
...
const filterLinesTable = () => {
  ...
  $.each(linhas, function(index, linha) {
    ...
    try {
      ...
      //USERS
      if(filtersTable.users.length > 0) {
        console.log('USER',filtersTable.users)
      ...
      if(filtersTable.date != '') {
        ...
        if(DATE){
          ...
          console.log('DATA INICIO', dataInit)
...
function filterTable() {
  ...
  if(!filter in [0,1,2,3,4,5]) return //verifica o tipo do filtro
 ...
 */
  ...
  const showResult = async (e) => {
    ...
    for (const key in e) {
      ...
      var status = statusProblem(e[key].problem_status)

      $('#problems-tbody').append(
         `<tr data-id='${e[key].problem_id}' data-date='${dateFormat(e[key].problem_opening_date,2)}' data-late-timestamp='${e[key].problem_late_timestamp ?? '99999999999999999'}' data-status='${e[key].problem_status}' data-users='${JSON.stringify(e[key].problem_users)}' data-groups='${JSON.stringify(e[key].problem_groups)}' onclick="problem_tr(this)">
           ...
           <td>${e[key].problem_id}</td>
           <td>${maxLength(e[key].problem_name)}</td>
           <td>${maxLength(groupsIncluded(e[key].problem_groups))}</td>
           <td>${statusProblem(e[key].problem_status)}</td>
           <td>${dateFormat(e[key].problem_opening_date,1)}</td>
           <td>${e[key].problem_late_time}</td>
         </tr>`
      )
    ...
    $('#solved-problems-counter').html(statusCounter?.Solucionado || 0)
    $('#canceled-problems-counter').html(statusCounter?.Fechado || 0)
    $('#attendance-problems-counter').html(Number(statusCounter?.Ematendimentoatribuido || 0)) + Number((statusCounter?.Ematendimentoplanejado|| 0))
    $('#total-problems-counter').html(i)
  }

  const loadTable = async () => {
    try {

      const response = await fetchApi('problems/')
      sessionStorage.setItem("problems_data", JSON.stringify(response.content))
...
function orderTable(coluna) {
  ...
  linhas.sort(function(a, b) {
    ...
    if(coluna == 6){
      var valorA = $(a).attr('data-late-timestamp');
      var valorB = $(b).attr('data-late-timestamp');
